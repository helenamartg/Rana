...
# HW (51sp)
############
hw <- cbind(morpho$sp, morpho$res_HW)
rownames(hw) <- hw[,1]
hw[,1] == tr51$tip.label
...
# HL (45sp)
############
hl <- cbind(morpho$sp, morpho$res_HL)
rownames(hl) <- hl[,1]
hl[,1] == tr51$tip.label
hl <- hl[tr51$tip.label,]
hl <- hl[,-1]
hl <- as.data.frame(hl)
hl <- na.omit(hl)
...
tr51
tr45 <- get_subtree_with_tips(tr51, rownames(hl))
tr45 <- tr45$subtree
# write.tree(tr45, "tree_hl.tre")
rownames(hl) == tr45$tip.label  # CHECK
...
# FL (43sp)
############
fl <- cbind(morpho$sp, morpho$res_FL)
rownames(fl) <- fl[,1]
fl[,1] == tr51$tip.label
...
tr51
tr43 <- get_subtree_with_tips(tr51, rownames(fl))
tr43 <- tr43$subtree
# write.tree(tr43, "tree_fl.tre")
rownames(fl) == tr43$tip.label  # CHECK
...
# TL (46sp)
############
tl <- cbind(morpho$sp, morpho$res_TL)
rownames(tl) <- tl[,1]
tl[,1] == tr51$tip.label
...
tr51
tr46 <- get_subtree_with_tips(tr51, rownames(tl))
tr46 <- tr46$subtree
# write.tree(tr46, "tree_tl.tre")
rownames(tl) == tr46$tip.label  # CHECK
