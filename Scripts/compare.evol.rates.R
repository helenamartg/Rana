...
rownames(clim) == morpho$sp
clim_red <- clim[morpho$sp,]
clim_red <- clim_red[,-grep("sd", colnames(clim_red))]
clim_red <- clim_red[,c(3:29)]


## Scale climatic dataset for ALL
clim_red  <- as.data.frame(scale(clim_red , center=T, scale = T))
clim_red  <- as.matrix(clim_red)


# Check order
rownames(clim_red) == morpho$sp
rownames(clim_red) == phylo37$tip.label
...
# plot
library(ggplot2)
p1 <- ggplot(evo, aes(x=groups, y=sigma)) +
  theme_classic() +
  ylab("sigma") +
...
p2 <- ggplot(evo.svl, aes(x=groups, y=sigma)) +
  theme_classic() +
  ylab("sigma") +
  geom_point(size=5, colour=c("gold", "darkorchid"))
