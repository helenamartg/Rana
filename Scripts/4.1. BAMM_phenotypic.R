...
setwd("C:/Users/helen/Desktop/MASTER_UB/TFM/4Publication/2nd_Revision/Reanalisis/BAMM")
...
mcmcout <- read.csv("mcmc_out_trait_svl.txt", header=T)
...
edata_trait <- getEventData("tree_svl.tre", eventdata = "event_data_trait_svl.txt", 
                            burnin=0.5, type = "trait")

postfile <- "mcmc_out_trait_svl.txt"
...
cset$number.distinct  # 4761 distinct shift configurations
...
tr59 <- read.tree("tree_svl.tre")
par(mar=c(6,6,6,6))
layout(1,1)
st <- max(branching.times(tr59))
plotRateThroughTime(edata_trait, intervalCol="black", 
                    avgCol="black",start.time=st, ylim=c(0,0.1), cex.axis=1)
text(x=30, y= 0.08, label="Rana (59 sp)", font=4, cex=1.5, pos=4)

# Separate radiations
#====================
allrates <- getCladeRates(edata_trait)
mean(allrates$beta) # tasa fenotípica = 0.008247304
...
american <- getCladeRates(edata_trait, node=86)
mean(american$beta)  # Mayor tasa fenotípica que todo el conjunto (0.009665666)
quantile(american$beta, c(0.05, 0.95))

mrca <- 86

# Extraer MRCA Euroasíaticas
eurasian <- getCladeRates(edata_trait, node=61)
mean(eurasian$beta)
...
plotRateThroughTime(edata_trait, intervalCol="#bf90e8", 
                    avgCol="darkorchid", xlim = c(st,0), ylim=c(0,0.04), cex.axis=1,
                    node = mrca, nodetype="exclude")

plotRateThroughTime(edata_trait, intervalCol="#f5f59f", 
                    avgCol="gold", xlim = c(st,0), ylim=c(0,0.04), cex.axis=1,
...
plot(computeBayesFactors(mcmcout, expectedNumberOfShifts=1, burnin=0.5))
...
# Extraer MRCA americanas
american <- getCladeRates(edata_hw, node=44)
mean(american$beta)  # tasa fenotípica = 0.003413523
mrca <- 44
...
# Extraer MRCA americanas
american <- getCladeRates(edata_hl, node=56)
mean(american$beta)  # tasa fenotípica = 0.001977154
mrca <- 56
...
# Extraer MRCA americanas
american <- getCladeRates(edata_fl, node=53)
mean(american$beta)  # tasa fenotípica = 0.001537839
mrca <- 53
...
# Extraer MRCA americanas
american <- getCladeRates(edata_tl, node=57)
mean(american$beta)  # tasa fenotípica = 0.001394344
mrca <- 57
...
diversity_data$Species <- rownames(diversity_data)
colnames(diversity_data) <-  c("rate", "Species")

#Import traits 
dt59 <- read.csv("C:/Users/helen/Desktop/MASTER_UB/TFM/4publication/2nd_Revision/Reanalisis/Data/dt_59sp.csv")
diversity_data$Species == dt59$Species
diversity_data <- diversity_data[dt59$Species,]
diversity_data$radiation <- dt59$phylo_subtree
diversity_data$state <- dt59$simpleState
diversity_data$ecotype <- as.factor(dt59$ECOTYPE)
...
subgen$sp == diversity_data$Species
subgen <- subgen[diversity_data$Species,]

diversity_data$subgen <- subgen$Subgenera
div2 <- diversity_data[diversity_data$Species!="Rana_weiningensis",]
div2 <- div2[div2$Species!="Rana_shuchinae",]
...
par(mar=c(4,4,4,4))
plot.bammdata(edata_trait, lwd=2.5, method="phylogram", pal="RdBu", legend=T, labels=T, font=3)
addBAMMshifts(best, cex=1.5, col="white", bg="red")
