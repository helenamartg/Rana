...
lm.SVL <- lm.rrpp(svl ~ radiation, data = rrpp.global, SS.type = "III")  # Without phylogeny (OLS)
anova(lm.SVL)   # Not significant¿??¿????
...
lm.state <- lm.rrpp(svl ~ state, data = rrpp.global, SS.type = "III") # Without phylogeny (OLS)
anova(lm.state)
...
lm.biome <- lm.rrpp(svl ~ biome, data=rrpp.global, SS.type="III", Cov = cov.rana) # PHylogenetic (OLS)
anova(lm.biome)
...
lm.radstate <- lm.rrpp(svl ~ radiation*state, data = rrpp.global, SS.type = "III")
anova(lm.radstate)
...
table(interaction(rrpp.dt$ecotype, rrpp.dt$biome))
table(rrpp.dt$ecotype)
table(interaction(rrpp.dt$biome, rrpp.dt$radiation))
