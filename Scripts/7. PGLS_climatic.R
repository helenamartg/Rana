...
ggplot(coef.svl.am, aes(x=vars, y=svl)) +
  geom_bar(stat="identity", aes(fill = ifelse(svl <0, "red", "blue"))) + 
  theme(legend.position = 'none') + labs(x= "", y="log(svl)") +
...
library(cowplot)
plot_grid(p2, p3, align="hv", nrow=2, ncol=1)
...
# REORDER
dim(mat)
mat <- mat[,c(27:25, 21:4, 24:22, 3:1)]
mat.ea <- mat.ea[,c(27:25, 21:4, 24:22, 3:1)]
mat.am <- mat.am[,c(27:25, 21:4, 24:22, 3:1)]
