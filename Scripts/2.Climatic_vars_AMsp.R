...
# proj4string(distAM) <- CRS("+init=epsg:4326")
...
# NDVI
#######
ndvi.vals <- extract(ndvi, distAM, na.rm=T)
names(ndvi.vals) <- distAM@data$sci_name
str(ndvi.vals)
...
# Aridity
##########
arid.vals <- extract(arid, distAM, na.rm=T)
names(arid.vals) <- distAM@data$sci_name
str(arid.vals)
...
# PET
##########
pet.vals <- extract(pet, distAM, na.rm=T)
names(pet.vals) <- distAM@data$sci_name
str(pet.vals)
...
# elev
#########
elev.vals <- extract(elev, distAM, na.rm=T)
names(elev.vals) <- distAM@data$sci_name
str(elev.vals)
...
# tmax
#########
tmax.vals <- extract(tmax, distAM, na.rm=T)
names(tmax.vals) <- distAM@data$sci_name
str(tmax.vals)
...
# tmin
#########
tmin.vals <- extract(tmin, distAM, na.rm=T)
names(tmin.vals) <- distAM@data$sci_name
str(tmin.vals)
...
# TAVG
########
tavg.vals <- extract(tavg, distAM, na.rm=T)
names(tavg.vals) <- distAM@data$sci_name
str(tavg.vals)
...
# VAPR
########
vapr.vals <- extract(vapr, distAM, na.rm=T)
names(vapr.vals) <- distAM@data$sci_name
str(vapr.vals)
...
# WIND
##########
wind.vals <- extract(wind, distAM, na.rm=T)
names(wind.vals) <- distAM@data$sci_name
str(wind.vals)
...
prec.vals <- extract(sum.prec, distAM, na.rm=T)
names(prec.vals) <- distAM@data$sci_name
str(prec.vals)
...
srad.vals <- extract(sum.srad, distAM, na.rm=T)
names(srad.vals) <- distAM@data$sci_name
str(srad.vals)
...
env95 <- as.data.frame(cbind(ARIDavg, ARIDmax, ARIDmin, aridSD,
                             ...
                             SRADavg, SRADmax, SRADmin, SRADsd))
colnames(env95)
rownames(env95) <- distAM@data$sci_name
rownames(env95)

# write.csv(env95, "env_vars_AM_sp.csv")
